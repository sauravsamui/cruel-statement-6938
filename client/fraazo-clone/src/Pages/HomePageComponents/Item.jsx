...
const Item = ({item,styles}) => {
  ...
  return (
    ...
    <div key={item._id} className={styles.card}>
              <Link to={`/productsdetails/${item._id}`}><div className={styles.cardTop}>
                <img style={{ backgroundColor: "rgb(249,249,249)" }} src={item.src} alt={item.name} />
              </div></Link> 
               <div className={styles.cardBottom}>
               <Link to={`/productsdetails/${item._id}`}><p style={{ marginLeft: "5px", fontSize:"14px",color:"#1a201e" }}>{item.name}</p></Link>
                <div style={{ marginTop: "10px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                <Link to={`/productsdetails/${item._id}`}> <div>
